...
import { getToken, saveToken, useApi } from "./api";
      ...
      <div className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <span className="workspace-avatar" title="本地工作区">
              ML
            </span>
          </div>
        </header>
          ...
          <div className="modal-body">
            <div className="guide-steps">
              {[
                {
                  title: "接入模型服务",
                  text: "模型中心预置规则模拟服务，可直接体验。连接真实模型时，选择 Ollama 或 OpenAI 兼容服务，填写模型名称和服务地址。",
                  page: "models",
                },
                {
                  title: "运行一条工单",
                  text: "在工单体验台选择模型、输入文本，查看分类与字段提取结果，以及本次调用的请求编号。",
                  page: "playground",
                },
                {
                  title: "创建对比评测",
                  text: "选择数据集、模型和测试集划分，平台会记录参数快照、数据指纹和指标。演示规则结果不代表真实大模型能力。",
                  page: "evaluations",
                },
                {
                  title: "查看问题与导出报告",
                  text: "在评测详情逐条对比标准答案；在请求日志中查看失败原因。报告可导出 Markdown、JSON 或 CSV。",
                  page: "requests",
                },
              ].map((step, index) => (
                <button
...
function Settings({
  ...
  return (
    <Modal
      title="连接设置"
      description="配置当前浏览器会话访问工作区所需的密钥。"
      onClose={onClose}
    >
      <form
        onSubmit={(event) => {
